...
def get_auth_token():
    try:
        ...
        resp = requests.post(url=SPOTII_IP+"/merchant/authentication/",data=json.dumps(auth_key_info),headers=headers)
...
def process_order_checkout(generic_cart_obj, is_fast_cart, reference):
    try:
        ...
        resp = requests.post(url=SPOTII_IP+"/checkouts/", data=json.dumps(order_info), headers=headers)
        resp = resp.json()
...
def on_approve_capture_order(order_reference):
    try:
        ...
        resp = requests.post(url=SPOTII_IP+"/orders/"+order_reference+"/capture/", data={}, headers=headers)
        resp = resp.json()
