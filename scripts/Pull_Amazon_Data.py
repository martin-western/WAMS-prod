...
filename = "scripts/Pricing.xlsx"

dfs_list = []
dfs_list.append(pd.read_excel(filename, sheet_name=None)["Storck"])
dfs_list.append(pd.read_excel(filename, sheet_name=None)["Nestle"])
dfs_list.append(pd.read_excel(filename, sheet_name=None)["Guylian"])
dfs_list.append(pd.read_excel(filename, sheet_name=None)["Ferrero"])
dfs_list.append(pd.read_excel(filename, sheet_name=None)["Hershey"])

index_list= [9,8,8,8,5]
index=-1
for dfs in dfs_list:
    index +=1
    try:
        ...
        for i in range(rows):
            ...
            try:
                ...
                barcode_string = str(int(dfs.iloc[i,index_list[index]]))

                if len(barcode_string)==10:
                    product_id_type = "ASIN"
                elif len(barcode_string)==12:
                    product_id_type = "UPC"
                elif len(barcode_string)==13:
                    product_id_type = "EAN"
                else:
                    barcode_string=""
                    product_id_type = ""
                ...
                if flag==1 and len(id_list) !=0:
                    ...
                    if isinstance(parsed_resposne,list):
                        ...
                        for j in range(len(parsed_resposne)):
                            ...
                            if index==4 :
                                find = dfs[dfs["CODE"]==id_list[j]].index[0]
                            else:
                                find = dfs[dfs["Barcode product"]==id_list[j]].index[0]
...
writer = pd.ExcelWriter('Pricing_New.xlsx', engine='xlsxwriter')
