...
def create_wishlist_report(filename, uuid, brand_list, custom_permission_obj):
    ...
    dealshub_user_objs = DealsHubUser.objects.filter(pk__in=UnitWishList.objects.filter(product__product__base_product__brand__name__in=brand_list, wish_list__location_group__in=location_group_objs).values_list('owner', flat=True).distinct())
...
def create_abandoned_cart_report(filename, uuid, brand_list, custom_permission_obj):
    ...
    dealshub_user_objs = DealsHubUser.objects.filter(pk__in=UnitCart.objects.filter(product__product__base_product__brand__name__in=brand_list, cart__location_group__in=location_group_objs).values_list('owner', flat=True).distinct())
