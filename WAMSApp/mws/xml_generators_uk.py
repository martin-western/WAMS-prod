...
def generate_xml_for_post_product_data_amazon_uk(product_pk_list,seller_id):
    ...
    try:
        ...
        for product_pk in product_pk_list:
            ...
            amazon_uk_product["recommended_browse_nodes"] = get_recommended_browse_node(seller_sku,"Amazon UK", product_obj.base_product.brand)
            product_obj.channel_product.amazon_uk_product_json = json.dumps(amazon_uk_product)
            ...
            if(amazon_uk_product["recommended_browse_nodes"] != ""):
                xml_string += """<RecommendedBrowseNode>"""+amazon_uk_product["recommended_browse_nodes"]+"""</RecommendedBrowseNode>"""
        ...
        logger.error("Generating XML UK: %s at %s", e, str(exc_tb.tb_lineno))
        return ""


def generate_xml_for_partial_update_product_amazon_uk(product_pk_list,seller_id):
    try:
         # Check if Cached
        xml_string = """<?xml version="1.0"?>
                        <AmazonEnvelope xsi:noNamespaceSchemaLocation="amzn-envelope.xsd"
        ...
        for product_pk in product_pk_list:
            ...
            amazon_uk_product_dict = json.loads(product_obj.channel_product.amazon_uk_product_json)
            product_name = amazon_uk_product_dict["product_name"]
            product_description = amazon_uk_product_dict["product_description"]
            ...
            amazon_uk_product_dict["recommended_browse_nodes"] = get_recommended_browse_node(seller_sku,"Amazon UK", product_obj.base_product.brand)
            product_obj.channel_product.amazon_uk_product_json = json.dumps(amazon_uk_product_dict)
            product_obj.channel_product.save()
            ...
            for attribute in amazon_uk_product_dict["product_attribute_list"]:
            ...
            if(amazon_uk_product_dict["recommended_browse_nodes"] != ""):
                xml_string += """<RecommendedBrowseNode>"""+amazon_uk_product_dict["recommended_browse_nodes"]+"""</RecommendedBrowseNode>"""
        ...
        print(str(e))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("Generating XML for Partial Update Product UK: %s at %s", e, str(exc_tb.tb_lineno))
        return ""


def generate_xml_for_price_data_amazon_uk(product_pk_list,seller_id):
    ...
    try:
        ...
        for product_pk in product_pk_list:
            ...
            seller_sku = product_obj.base_product.seller_sku
            amazon_uk_product = json.loads(product_obj.channel_product.amazon_uk_product_json)
            price = amazon_uk_product["now_price"]
        ...
        print(str(e))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("Price Update XML UK: %s at %s", e, str(exc_tb.tb_lineno))
        return ""

def generate_xml_for_inventory_data_amazon_uk(product_pk_list,seller_id):
    ...
    try:
        ...
        for product_pk in product_pk_list:
            ...
            seller_sku = product_obj.base_product.seller_sku
            amazon_uk_product = json.loads(product_obj.channel_product.amazon_uk_product_json)
            quantity = amazon_uk_product["stock"]
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("Inventory Update XML UK: %s at %s", e, str(exc_tb.tb_lineno))
        return ""
...
def generate_xml_for_product_image_amazon_uk(product_pk_list,seller_id):
    ...
    try:
        ...
        for product_pk in product_pk_list:
            ...
            try:
                ...
                image_type = "Main"
                image_url = MainImages.objects.get(product=product_obj,channel__name="Amazon UK").main_images.all()[0].image.image.url
            ...
            try:
                ...
                for image_bucket in SubImages.objects.get(product=product_obj,channel__name="Amazon UK").sub_images.all()[:8] :
        ...
        print(str(e))
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("Generating ImagesFeed XML UK: %s at %s", e, str(exc_tb.tb_lineno))
        return ""