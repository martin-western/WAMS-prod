...
class SaveVarientProductAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if request.user.has_perm('WAMSApp.change_product') == False:
                logger.warning("SaveVarientProductAPI Restricted Access!")
                response['status'] = 403
                return Response(data=response)

            data = request.data
            logger.info("SaveVarientProductAPI: %s", str(data))
...
SaveVarientProduct = SaveVarientProductAPI.as_view()