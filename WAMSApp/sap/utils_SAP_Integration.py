...
def get_user_input_requirement(order_obj, response):
    ...
    for unit_order_obj in UnitOrder.objects.filter(order=order_obj).exclude(current_status_admin="cancelled"):
        ...
        if stock_price_information["status"] == 500:
            ...
            logger.error("get_user_input_requirement: fetch prices and stock gave 500!")
...
def get_modal_info_list(order_obj, user_input_requirement, is_resend=False):
    '''
...
def get_order_information(order_obj, company_code_obj, grouped_unit_orders, response, user_input_requirement, user_input_sap, is_resend=False):
    '''
    Returns the order_information with respect to a specific brand [implicitly passed as the company_code_obj]
    '''
    is_final_response = True
    order_information = {}
    ...
    if is_b2b==True:
        ...
        if not(is_resend) and order_information["trn"] == "":
            response["message"] = "TRN number is empty"
            response["status"] = 406
            return order_information, is_final_response
    order_information["items"] = []
    
    for unit_order_obj in grouped_unit_orders[company_code_obj.get_brand_name()]:
